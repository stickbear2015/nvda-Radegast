...
TIMESTAMPREGEX = re.compile(r"^(\d+-\d+-\d+ \[\d+:\d+:\d+\]|\[\d+:\d+\]) ")
...
def loadAddonConfig(fileName):
	...
	if not ADDON_CONFIG:
		...
		ADDON_CONFIG = ConfigObj(path, configspec=BytesIO(ADDON_CONFIG_SPEC), default_encoding="utf-8", encoding="utf-8", stringify=True)
